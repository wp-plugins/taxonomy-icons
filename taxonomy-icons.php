 ...
 * Define constants.
 */
define( 'TAX_ICONS_VERSION', '1.0.0' );
...
require_once( TAX_ICONS_PATH . 'lib/better-font-awesome-library/better-font-awesome-library.php' );
...
add_action( 'init', 'tax_icons_init' );
/**
 * Initialize plugin.
 *
 * @since  1.0.0
 */
function tax_icons_init() {
	$locale = apply_filters( 'plugin_locale', get_locale(), 'taxonomy-icons' );
	load_textdomain( 'taxonomy-icons', WP_LANG_DIR . '/taxonomy-icons/taxonomy-icons-' . $locale . '.mo' );
...
add_shortcode( 'tax_icon', 'tax_icons_term_icon_shortcode' );
/**
...
add_action( 'admin_enqueue_scripts', 'tax_icons_admin_scripts_and_styles' );
/**
 * Set up admin scripts and styles.
 *
 * @since  1.0.0
 */
function tax_icons_admin_scripts_and_styles() {

	// Admin jQuery
	wp_enqueue_script( 'taxonomy-icons', TAX_ICONS_URL . '/includes/js/taxonomy-icons-admin.js', true  );

	// Admin CSS
	wp_enqueue_style( 'taxonomy-icons', TAX_ICONS_URL . '/includes/css/taxonomy-icons-admin.css', true  );
...
function tax_icons_get_icon_select_label() {
	return '<label for="term_meta[term_icon]">' . __( 'Taxonomy Icon', 'taxonomy-icons' ) . '</label>';
}
...
function tax_icons_get_icon_select( $term_meta = array() ) {
	...
	<select name="term_meta[term_icon]" id="term_meta[term_icon]" class="tax-icons-icon-selector">
		<option value="none" <?php selected( $value, 'none' ); ?>><?php _e( 'None', 'taxonomy-icons' ); ?></option>
		<?php
...
function tax_icons_get_icon_select_description() {
	return __( 'Choose an icon to associate with this taxonomy term.', 'taxonomy-icons' );
}
...
function tax_icons_add_admin_icon_column_heading( $defaults ) {
    $defaults['taxonomy_icon'] = __( 'Icon', 'taxonomy-icons' );
    return $defaults;
}
